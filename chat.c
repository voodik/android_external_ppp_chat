...
void msgf __V((const char *fmt, ...))
{
    ...
    if (to_log){
#ifdef ANDROID_CHANGES		
	__android_log_write(syslog_to_android[LOG_INFO], LOG_TAG, line);
#else
...
void fatal __V((int code, const char *fmt, ...))
{
    ...
    if (to_log){
#ifdef ANDROID_CHANGES		
	__android_log_write(syslog_to_android[LOG_ERR], LOG_TAG, line);
#else
...
/*
    ...
    strcat(tmp,"\r");
    strcat(tmp,"\0");
    fprintf(stderr,"the cmmand will send is===%s\n",tmp);
     written =write(1,tmp,strlen(tmp));
    if (written < 0) {
        return 0;
    }
    alarm(0);
